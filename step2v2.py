...
#REVISED Step 2.3:
# create a revised version of UniqueTokensPerReview with a unique ID for each review
# previous one did not have a unique ID for each review
# (the workflow.xlsx is misleading)
# this table is necessary for step 5
# we cannot simply look up with reviewerID because one reviewer can write multiple reviews in our dataset
c.execute("DROP TABLE IF EXISTS `UniqueTokensPerReviewWithID`")
create_UniqueTokensPerReviewWithID = "CREATE TABLE UniqueTokensPerReviewWithID(" \
                         "recordID INT NOT NULL, " \
                         "reviewID INT NOT NULL, " \
                         "reviewerID VARCHAR(255), " \
                         "token VARCHAR(255), " \
                         "count INT NOT NULL, " \
                         "PRIMARY KEY (`recordID`))"
insert_UniqueTokensPerReviewWithID = "INSERT INTO UniqueTokensPerReviewWithID "\
    "(recordID, reviewID, reviewerID, token, count) VALUES "\
    "(%s, %s, %s, %s, %s)"
c.execute(create_UniqueTokensPerReviewWithID)
...
for eachRow in rows:
    ...
    for token in tokensPerReview:
        try:
            tokenPerReviewID += 1
            c.execute(insert_UniqueTokensPerReviewWithID, (tokenPerReviewID, reviewID, reviewerID, token, tokensPerReview[token]))
        except mysql.connector.errors.DataError as e:
