# Yelp DLA project step 4, version 2
...
################################################################################
#REVISiON
#first create uniqueBigramsPerReviewWithID
#REVISED Step2.3: create UniqueTokensPerReviewWithID table
#previous version does not have unique ID for each review
c.execute("DROP TABLE IF EXISTS `UniqueBigramsPerReviewWithID`")
create_UniqueBigramsPerReviewWithID = "CREATE TABLE UniqueBigramsPerReviewWithID(" \
                         "recordID INT NOT NULL, " \
                         ...
                         "count INT NOT NULL, " \
                         "PRIMARY KEY (`recordID`))"
insert_UniqueBigramsPerReviewWithID = "INSERT INTO UniqueBigramsPerReviewWithID "\
    "(recordID, reviewID, reviewerID, bigram, count) VALUES "\
    "(%s, %s, %s, %s, %s)"


c.execute(create_UniqueBigramsPerReviewWithID)
...
for eachRow in rows:
    ...
    for phrase in bigramsPerReview:
        try:
            bigramPerReviewID += 1
            c.execute(insert_UniqueBigramsPerReviewWithID, (bigramPerReviewID, reviewID, reviewerID, phrase, bigramsPerReview[phrase]))
...
################################################################################
#REVISION
#first create uniqueTrigramsPerReviewWithID
#Step2.3: create UniqueTokensPerReviewWithID table
c.execute("DROP TABLE IF EXISTS `UniqueTrigramsPerReviewWithID`")
create_UniqueTrigramsPerReviewWithID = "CREATE TABLE UniqueTrigramsPerReviewWithID(" \
                         "recordID INT NOT NULL, " \
                         "reviewID INT NOT NULL, " \
                         "reviewerID VARCHAR(255), " \
                         "trigram VARCHAR(255), " \
                         "count INT NOT NULL, " \
                         "PRIMARY KEY (`recordID`))"
insert_UniqueTrigramsPerReviewWithID = "INSERT INTO UniqueTrigramsPerReviewWithID "\
    "(recordID, reviewID, reviewerID, trigram, count) VALUES "\
    "(%s, %s, %s, %s, %s)"


c.execute(create_UniqueTrigramsPerReviewWithID)
c.execute("SELECT * FROM TABLE2")
rows = c.fetchall()
...
for eachRow in rows:
    ...
    for phrase in trigramsPerReview:
        try:
            ...
            c.execute(insert_UniqueTrigramsPerReviewWithID, (trigramPerReviewID, reviewID, reviewerID, phrase, trigramsPerReview[phrase]))
        except mysql.connector.errors.DataError as e:
